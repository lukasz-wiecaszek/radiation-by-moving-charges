...
q1 = charge.OscillatingCharge(r0, args.amplitude, +args.frequency, +e)
q2 = charge.OscillatingCharge(r0, args.amplitude, -args.frequency, -e)
...
t = np.reciprocal(args.frequency) / 4
scatter1 = ax.scatter(q1.position(t)[0], q1.position(t)[1], s=3, c="red", marker="o")
scatter2 = ax.scatter(q2.position(t)[0], q2.position(t)[1], s=3, c="blue", marker="o")
...
dt = 2 * np.reciprocal(args.frequency) / frames
def update(frame):
    ...
    t = frame * dt
    scatter1.set_offsets([q1.position(t)[0], q1.position(t)[1]])
    scatter2.set_offsets([q2.position(t)[0], q2.position(t)[1]])
    time.set_text(f"t = {t:.2e} s")
...
if args.animation:
    ...
    animation.save(
        f"oscillating-electric-dipole-s-field-{args.frequency:.2e}Hz.mp4",
        writer = mpla.FFMpegWriter(fps = fps))
else:
    plt.savefig(
        f"oscillating-electric-dipole-s-field-{args.frequency:.2e}Hz.png",
